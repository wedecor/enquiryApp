...
void main() {
  ...
  group('StatusInlineControl', () {
    ...
    testWidgets('assigned staff: change + Undo => two writes', (tester) async {
      ...
      await tester.tap(find.text('quotation_sent').last);
      await tester.pump();

      verify(
        () => repo.updateStatus(id: 'E2', nextStatus: 'quotation_sent', userId: 'staff1'),
      ).called(1);

      if (find.text('Undo').evaluate().isNotEmpty) {
        // Programmatically invoke SnackBarAction to avoid off-screen tap issues in tests
        final actionWidget = tester.widget<SnackBarAction>(find.byType(SnackBarAction));
        actionWidget.onPressed();
        await tester.pump();
        verify(
          () => repo.updateStatus(id: 'E2', nextStatus: 'in_talks', userId: 'staff1'),
        ).called(1);
      }
    });
  });
