...
class DatabaseSetupService {
  ...
  Future<void> _createCollections() async {
    ...
    await _firestore.collection('dropdowns').doc('event_types').collection('items').doc('_temp').set({
      'temp': true,
    });
    ...
    await _firestore.collection('dropdowns').doc('payment_statuses').collection('items').doc('_temp').set({
      'temp': true,
    });
    ...
    await _firestore.collection('dropdowns').doc('event_types').collection('items').doc('_temp').delete();
    await _firestore.collection('dropdowns').doc('statuses').collection('items').doc('_temp').delete();
    await _firestore.collection('dropdowns').doc('payment_statuses').collection('items').doc('_temp').delete();
  ...
  Future<void> _initializePaymentStatuses() async {
    final batch = _firestore.batch();
    final collectionRef = _firestore.collection('dropdowns').doc('payment_statuses').collection('items');
  ...
  Future<Map<String, bool>> verifyDatabaseStructure() async {
    ...
    try {
      ...
      final enquiriesSnapshot = await _firestore.collection('enquiries').limit(1).get();
      results['enquiries_collection'] = true;
      
      final dropdownsSnapshot = await _firestore.collection('dropdowns').limit(1).get();
      results['dropdowns_collection'] = true;
      
      // Check dropdown subcollections
      final eventTypesSnapshot = await _firestore
      ...
      final paymentStatusesSnapshot = await _firestore
          .collection('dropdowns')
