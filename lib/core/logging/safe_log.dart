...
String _redact(String key, Object? value) {
  ...
  const sensitivePatterns = [
    'token', 'authorization', 'cookie', 'secret', 'key', 
    'password', 'auth', 'bearer', 'jwt', 'session', 
    'credential', 'private', 'fcm', 'vapid'
  ];
...
void safeLogObject(String label, Object? object) {
  ...
  const sensitivePatterns = [
    'token', 'authorization', 'secret', 'key', 'password'
  ];
  
  final containsSensitive = sensitivePatterns.any((pattern) => 
    objectStr.toLowerCase().contains(pattern)
  );
...
extension SafeLogMap on Map<String, Object?> {
  ...
  void safeLog(String label) => safeLog(label, this);
}
