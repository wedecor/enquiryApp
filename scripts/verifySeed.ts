...
async function main() {
  ...
  try {
    ...
    const expectedTotal = 28; // 7 statuses + 6 event_types + 4 priorities + 4 payment_statuses + 7 sources
    
    if (totalDropdowns >= expectedTotal) {
      results.push({ Check: 'Dropdown Count (28 expected)', Result: 'PASS' });
    } else {
      results.push({ Check: 'Dropdown Count (28 expected)', Result: 'FAIL', Details: `Only ${totalDropdowns}/${expectedTotal} dropdown items found` });
      allPassed = false;
    }
  } catch (error) {
    results.push({ Check: 'Dropdown Count (28 expected)', Result: 'FAIL', Details: `Error: ${error}` });
    allPassed = false;
  }
